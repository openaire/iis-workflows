"""
Integration test DAG for UnionDedupJob (with_report test variant).

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/common/uniondedup/with_report)
with an Airflow DAG that can be run in a Kubernetes environment.

Note: The Spark UnionDedupJob does not produce a report output. Only the union
data output is validated in this test.
...
@dag(
    dag_id="spark_uniondedup_with_report_test",
    dag_display_name="Integration test for UnionDedupJob (with_report variant)",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/uniondedup-with-report-test",
            type="string",
...
def spark_uniondedup_with_report_test():
    """Three-step integration test for UnionDedupJob (with_report variant)."""
...
spark_uniondedup_with_report_test()
