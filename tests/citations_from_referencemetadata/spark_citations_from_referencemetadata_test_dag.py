"""
Integration test DAG for CitationsFromReferenceMetadataTransformerJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/common/citations/from/referencemetadata/test)
...
Three-step test:
  1. prepare_input  – converts JSON fixtures into Avro data stores on HDFS
                      using SparkAvroTestProducer.
  2. run_job        – executes CitationsFromReferenceMetadataTransformerJob.
  3. validate_output – compares Avro output against expected JSON using
                       SparkAvroTestConsumer.
"""
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/common/citations/from/referencemetadata/test/data"
_INPUT_DOCUMENT_METADATA_JSON = f"{_DATA_ROOT}/input_document_metadata.json"
_OUTPUT_CITATIONS_JSON        = f"{_DATA_ROOT}/output_citations.json"

_SCHEMA_MERGED_WITH_ORIGINAL = "eu.dnetlib.iis.transformers.metadatamerger.schemas.ExtractedDocumentMetadataMergedWithOriginal"
_SCHEMA_CITATION             = "eu.dnetlib.iis.common.citations.schemas.Citation"
...
@dag(
    dag_id="spark_citations_from_referencemetadata_test",
    dag_display_name="Integration test for CitationsFromReferenceMetadataTransformerJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/citations-from-referencemetadata-test",
            type="string",
...
def spark_citations_from_referencemetadata_test():
    """Three-step integration test for CitationsFromReferenceMetadataTransformerJob."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            "-schemaClass",   _SCHEMA_MERGED_WITH_ORIGINAL,
            "-classpathJson", _INPUT_DOCUMENT_METADATA_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/doc_metadata",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run CitationsFromReferenceMetadataTransformerJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.common.citations.from.referencemetadata.CitationsFromReferenceMetadataTransformerJob",
        arguments=[
            "-input",  "{{ params.get('workingDir') }}/producer/doc_metadata",
            "-output", "{{ params.get('workingDir') }}/output/citations",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate output citations against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.common.spark.SparkAvroTestConsumer",
        arguments=[
            "-schemaClass",   _SCHEMA_CITATION,
            "-classpathJson", _OUTPUT_CITATIONS_JSON,
            "-hdfsInput",     "{{ params.get('workingDir') }}/output/citations",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_citations_from_referencemetadata_test()
