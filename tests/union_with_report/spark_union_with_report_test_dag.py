"""
Integration test DAG for UnionJob (with_report test variant).

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/common/union/with_report)
with an Airflow DAG that can be run in a Kubernetes environment.

Note: The Spark UnionJob does not produce a report output. Only the union data
output is validated in this test.

Three-step test:
  1. prepare_input  – converts JSON fixtures into Avro data stores on HDFS.
  2. run_job        – executes UnionJob.
  3. validate_output – compares Avro union output against expected JSON.
"""
...
_INPUT_ROOT  = "eu/dnetlib/iis/wf/transformers/common/union/input"
_OUTPUT_ROOT = "eu/dnetlib/iis/wf/transformers/common/union/expected_output"
...
@dag(
    dag_id="spark_union_with_report_test",
    dag_display_name="Integration test for UnionJob (with_report variant)",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/union-with-report-test",
            type="string",
...
def spark_union_with_report_test():
    """Three-step integration test for UnionJob (with_report variant)."""
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run UnionJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.common.union.UnionJob",
        arguments=[
            "-inputA",      "{{ params.get('workingDir') }}/producer/datastore_a",
            "-inputB",      "{{ params.get('workingDir') }}/producer/datastore_b",
            "-output",      "{{ params.get('workingDir') }}/union",
            "-schemaClass", _SCHEMA_IDENTIFIER_MAPPING,
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate union output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        ...
        arguments=[
            ...
            "-hdfsInput",     "{{ params.get('workingDir') }}/union",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
