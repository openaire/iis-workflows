"""
Integration test DAG for SkipExtractedWithoutMetaJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/metadataextraction/skip_extracted_without_meta/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes SkipExtractedWithoutMetaJob.
  3. validate_output – compares Avro document content output against expected JSON.
"""
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/metadataextraction/skip_extracted_without_meta/sampledataproducer/data"
...
@dag(
    dag_id="spark_skip_extracted_without_meta_test",
    dag_display_name="Integration test for SkipExtractedWithoutMetaJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/skip-extracted-without-meta-test",
            type="string",
...
def spark_skip_extracted_without_meta_test():
    """Three-step integration test for SkipExtractedWithoutMetaJob."""
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run SkipExtractedWithoutMetaJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.metadataextraction.skip_extracted_without_meta.SkipExtractedWithoutMetaJob",
        arguments=[
            ...
            "-outputDocumentContent", "{{ params.get('workingDir') }}/transformer/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate document content output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
...
spark_skip_extracted_without_meta_test()
