"""
Integration test DAG for PmcMetadataTransformerJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/ingest/pmc/metadata/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes PmcMetadataTransformerJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/ingest/pmc/metadata/sampledataproducer/data"
_INPUT_JSON  = f"{_DATA_ROOT}/input.json"
_OUTPUT_JSON = f"{_DATA_ROOT}/output.json"

_SCHEMA_PMC_METADATA      = "eu.dnetlib.iis.ingest.pmc.metadata.schemas.ExtractedDocumentMetadata"
...
@dag(
    dag_id="spark_ingest_pmc_metadata_test",
    dag_display_name="Integration test for PmcMetadataTransformerJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/ingest-pmc-metadata-test",
            type="string",
...
def spark_ingest_pmc_metadata_test():
    """Three-step integration test for PmcMetadataTransformerJob."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            "-schemaClass",   _SCHEMA_PMC_METADATA,
            "-classpathJson", _INPUT_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/input",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run PmcMetadataTransformerJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.ingest.pmc.metadata.PmcMetadataTransformerJob",
        arguments=[
            "-input",  "{{ params.get('workingDir') }}/producer/input",
            "-output", "{{ params.get('workingDir') }}/transformer/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate transformer output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        ...
        arguments=[
            ...
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformer/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_ingest_pmc_metadata_test()
