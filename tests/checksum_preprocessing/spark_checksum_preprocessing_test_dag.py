"""
Integration test DAG for ChecksumPreprocessingJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/metadataextraction/checksum/preprocessing/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes ChecksumPreprocessingJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/metadataextraction/checksum/preprocessing/sampledataproducer/data"
_INPUT_JSON  = f"{_DATA_ROOT}/input_document_content_url.json"
_OUTPUT_JSON = f"{_DATA_ROOT}/output_document_content_url.json"
...
@dag(
    dag_id="spark_checksum_preprocessing_test",
    dag_display_name="Integration test for ChecksumPreprocessingJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/checksum-preprocessing-test",
            type="string",
...
def spark_checksum_preprocessing_test():
    """Three-step integration test for ChecksumPreprocessingJob."""
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run ChecksumPreprocessingJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.metadataextraction.checksum.preprocessing.ChecksumPreprocessingJob",
        arguments=[
            "-input",  "{{ params.get('workingDir') }}/producer/document_content_url",
            "-output", "{{ params.get('workingDir') }}/transformer_metadataextraction_checksum_preprocessing/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate preprocessed output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        ...
        arguments=[
            ...
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformer_metadataextraction_checksum_preprocessing/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_checksum_preprocessing_test()
