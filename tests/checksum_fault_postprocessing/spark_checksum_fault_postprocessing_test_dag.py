"""
Integration test DAG for ChecksumFaultPostprocessingJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/metadataextraction/checksum/postprocessing/fault/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes ChecksumFaultPostprocessingJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/metadataextraction/checksum/postprocessing/fault/sampledataproducer/data"
_INPUT_DOCUMENT_CONTENT_URL_JSON = f"{_DATA_ROOT}/input_document_content_url.json"
_INPUT_FAULT_JSON                = f"{_DATA_ROOT}/input_fault.json"
_OUTPUT_FAULT_JSON               = f"{_DATA_ROOT}/output_fault.json"

_SCHEMA_DOCUMENT_CONTENT_URL = "eu.dnetlib.iis.importer.auxiliary.schemas.DocumentContentUrl"
_SCHEMA_FAULT                = "eu.dnetlib.iis.audit.schemas.Fault"
...
@dag(
    dag_id="spark_checksum_fault_postprocessing_test",
    dag_display_name="Integration test for ChecksumFaultPostprocessingJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/checksum-fault-postprocessing-test",
            type="string",
...
def spark_checksum_fault_postprocessing_test():
    """Three-step integration test for ChecksumFaultPostprocessingJob."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            ...
            "-schemaClass",   _SCHEMA_FAULT,
            "-classpathJson", _INPUT_FAULT_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/fault",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run ChecksumFaultPostprocessingJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.metadataextraction.checksum.postprocessing.fault.ChecksumFaultPostprocessingJob",
        arguments=[
            "-inputFault",               "{{ params.get('workingDir') }}/producer/fault",
            "-inputDocumentContentUrl",  "{{ params.get('workingDir') }}/producer/document_content_url",
            "-output",                   "{{ params.get('workingDir') }}/transformer_metadataextraction_checksum_postprocessing_fault/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate fault output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.common.spark.SparkAvroTestConsumer",
        arguments=[
            "-schemaClass",   _SCHEMA_FAULT,
            "-classpathJson", _OUTPUT_FAULT_JSON,
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformer_metadataextraction_checksum_postprocessing_fault/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_checksum_fault_postprocessing_test()
