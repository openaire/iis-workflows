"""
Integration test DAG for ExportCitationsTransformerJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/export/citations/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes ExportCitationsTransformerJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/export/citations/sampledataproducer/data"
_INPUT_CITATIONS_JSON  = f"{_DATA_ROOT}/common_citations.json"
_OUTPUT_CITATIONS_JSON = f"{_DATA_ROOT}/citations.json"
...
@dag(
    dag_id="spark_export_citations_test",
    dag_display_name="Integration test for ExportCitationsTransformerJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/export-citations-test",
            type="string",
...
def spark_export_citations_test():
    """Three-step integration test for ExportCitationsTransformerJob."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            ...
            "-classpathJson", _INPUT_CITATIONS_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/citation",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run ExportCitationsTransformerJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.export.citations.ExportCitationsTransformerJob",
        arguments=[
            "-input",  "{{ params.get('workingDir') }}/producer/citation",
            "-output", "{{ params.get('workingDir') }}/transformer_export_citations/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate export citations output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.common.spark.SparkAvroTestConsumer",
        arguments=[
            "-schemaClass",   _SCHEMA_CITATIONS,
            "-classpathJson", _OUTPUT_CITATIONS_JSON,
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformer_export_citations/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
