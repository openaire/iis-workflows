"""
Integration test DAG for IdReplacerJob (2-fields variant).

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/idreplacer/replacer_2_fields)
...
Three-step test:
  ...
  2. run_job        – executes IdReplacerJob replacing two id fields.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/idreplacer/replacer_2_fields/data"
_INPUT_CITATION_JSON   = f"{_DATA_ROOT}/citation.json"
_INPUT_ID_MAPPING_JSON = f"{_DATA_ROOT}/id_mapping.json"
_OUTPUT_JSON           = f"{_DATA_ROOT}/output.json"

# citationmatching.schemas.Citation (not common.citations)
_SCHEMA_CITATION        = "eu.dnetlib.iis.citationmatching.schemas.Citation"
_SCHEMA_IDENTIFIER_MAPPING = "eu.dnetlib.iis.common.schemas.IdentifierMapping"
...
@dag(
    dag_id="spark_idreplacer_2_fields_test",
    dag_display_name="Integration test for IdReplacerJob (2 id fields variant)",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/idreplacer-2-fields-test",
            type="string",
...
def spark_idreplacer_2_fields_test():
    """Three-step integration test for IdReplacerJob (2-fields variant)."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            ...
            "-classpathJson", _INPUT_CITATION_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/main",
            "-schemaClass",   _SCHEMA_IDENTIFIER_MAPPING,
            "-classpathJson", _INPUT_ID_MAPPING_JSON,
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/id_mapping",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run IdReplacerJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.idreplacer.IdReplacerJob",
        arguments=[
            "-input",             "{{ params.get('workingDir') }}/producer/main",
            "-inputIdMapping",    "{{ params.get('workingDir') }}/producer/id_mapping",
            "-output",            "{{ params.get('workingDir') }}/collapser_idreplacer/output",
            "-schemaClass",       _SCHEMA_CITATION,
            "-idFieldToReplace1", "sourceDocumentId",
            "-idFieldToReplace2", "destinationDocumentId",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate id-replaced output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        ...
        arguments=[
            ...
            "-hdfsInput",     "{{ params.get('workingDir') }}/collapser_idreplacer/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_idreplacer_2_fields_test()
