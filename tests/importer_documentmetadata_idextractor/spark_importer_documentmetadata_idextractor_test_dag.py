"""
Integration test DAG for DocumentMetadataIdExtractorJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/importer/documentmetadata/idextractor/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes DocumentMetadataIdExtractorJob.
  3. validate_output – compares Avro identifier output against expected JSON.
"""
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/importer/documentmetadata/idextractor/sampledataproducer/data"
_INPUT_JSON  = f"{_DATA_ROOT}/input_document_metadata.json"
_OUTPUT_JSON = f"{_DATA_ROOT}/output_document_id.json"
...
@dag(
    dag_id="spark_importer_documentmetadata_idextractor_test",
    dag_display_name="Integration test for DocumentMetadataIdExtractorJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/importer-documentmetadata-idextractor-test",
            type="string",
...
def spark_importer_documentmetadata_idextractor_test():
    """Three-step integration test for DocumentMetadataIdExtractorJob."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            ...
            "-hdfsOutput",    "{{ params.get('workingDir') }}/producer/document_metadata",
        ],
        spark_extra_conf={
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run DocumentMetadataIdExtractorJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.importer.documentmetadata.idextractor.DocumentMetadataIdExtractorJob",
        arguments=[
            "-inputDocumentMetadata", "{{ params.get('workingDir') }}/producer/document_metadata",
            "-outputIdentifier",      "{{ params.get('workingDir') }}/transformers_idextractor/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate identifier output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        ...
        arguments=[
            ...
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformers_idextractor/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
...
spark_importer_documentmetadata_idextractor_test()
