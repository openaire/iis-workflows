"""
Integration test DAG for ExistenceFilterJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/common/existencefilter/sampledataproducer)
...
Three-step test:
  1. prepare_input  – converts JSON fixtures into Avro data stores on HDFS
                      using SparkAvroTestProducer (two input datasets).
  2. run_job        – executes ExistenceFilterJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/common/existencefilter/sampledataproducer/data"
_EXISTENT_ID_JSON = f"{_DATA_ROOT}/existent_id.json"
_DATA_JSON        = f"{_DATA_ROOT}/data.json"
_FILTERED_JSON    = f"{_DATA_ROOT}/filtered.json"
...
@dag(
    dag_id="spark_existencefilter_test",
    dag_display_name="Integration test for ExistenceFilterJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/existencefilter-test",
            type="string",
...
def spark_existencefilter_test():
    """Three-step integration test for ExistenceFilterJob."""
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run ExistenceFilterJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.common.existencefilter.ExistenceFilterJob",
        arguments=[
            "-inputData",       "{{ params.get('workingDir') }}/producer/data",
            "-inputExistentId", "{{ params.get('workingDir') }}/producer/existent_id",
            "-outputFiltered",  "{{ params.get('workingDir') }}/existencefilter/filtered",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate filtered output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.common.spark.SparkAvroTestConsumer",
        arguments=[
            "-schemaClass",   _SCHEMA_DOCUMENT_CONTENT_URL,
            "-classpathJson", _FILTERED_JSON,
            "-hdfsInput",     "{{ params.get('workingDir') }}/existencefilter/filtered",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
