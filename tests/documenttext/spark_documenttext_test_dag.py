"""
Integration test DAG for DocumentTextTransformerJob.

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/metadataextraction/documenttext/sampledataproducer)
...
Three-step test:
  ...
  2. run_job        – executes DocumentTextTransformerJob.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/metadataextraction/documenttext/sampledataproducer/data"
_INPUT_JSON  = f"{_DATA_ROOT}/extr_metadata.json"
_OUTPUT_JSON = f"{_DATA_ROOT}/output_text.json"

_SCHEMA_EXTRACTED_METADATA = "eu.dnetlib.iis.metadataextraction.schemas.ExtractedDocumentMetadata"
_SCHEMA_DOCUMENT_TEXT      = "eu.dnetlib.iis.metadataextraction.schemas.DocumentText"
...
@dag(
    dag_id="spark_documenttext_test",
    dag_display_name="Integration test for DocumentTextTransformerJob",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/documenttext-test",
            type="string",
...
def spark_documenttext_test():
    """Three-step integration test for DocumentTextTransformerJob."""
    ...
    run_job = generate_spark_operator(
        task_id="run_job",
        task_display_name="Run DocumentTextTransformerJob (job under test)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.wf.transformers.metadataextraction.documenttext.DocumentTextTransformerJob",
        arguments=[
            "-input",  "{{ params.get('workingDir') }}/producer/extr_metadata",
            "-output", "{{ params.get('workingDir') }}/transformer_metadataextraction_documenttext/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":           "{{ params.get('sparkDriverMemory') }}",
    ...
    validate_output = generate_spark_operator(
        ...
        task_display_name="Validate document text output against expected JSON (SparkAvroTestConsumer)",
        jar="{{ params.get('JAR') }}",
        image="{{ params.get('SPARK_IMAGE') }}",
        main_class="eu.dnetlib.iis.common.spark.SparkAvroTestConsumer",
        arguments=[
            "-schemaClass",   _SCHEMA_DOCUMENT_TEXT,
            "-classpathJson", _OUTPUT_JSON,
            "-hdfsInput",     "{{ params.get('workingDir') }}/transformer_metadataextraction_documenttext/output",
        ],
        spark_extra_conf={
            "spark.driver.memory":      "{{ params.get('sparkDriverMemory') }}",
            "spark.executor.instances": "0",
