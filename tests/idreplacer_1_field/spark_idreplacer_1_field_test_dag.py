"""
Integration test DAG for IdReplacerJob (1-field variant).

Replaces the former Oozie-based test workflow
(eu/dnetlib/iis/wf/transformers/idreplacer/replacer_1_field)
...
Three-step test:
  ...
  2. run_job        – executes IdReplacerJob with a single id field.
...
_DATA_ROOT = "eu/dnetlib/iis/wf/transformers/idreplacer/replacer_1_field/data"
_INPUT_METADATA_JSON   = f"{_DATA_ROOT}/metadata.json"
...
@dag(
    dag_id="spark_idreplacer_1_field_test",
    dag_display_name="Integration test for IdReplacerJob (1 id field variant)",
    ...
    params={
        ...
        "workingDir": Param(
            default="hdfs://iis-cdh5-test-m1.ocean.icm.edu.pl:8020/tmp/marek.horst/idreplacer-1-field-test",
            type="string",
...
def spark_idreplacer_1_field_test():
    """Three-step integration test for IdReplacerJob (1-field variant)."""
    ...
    prepare_input = generate_spark_operator(
        ...
        arguments=[
            ...
            "-classpathJson", _INPUT_METADATA_JSON,
